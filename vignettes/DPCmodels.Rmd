...
`Disjunctive` are variants on the idea.  `OffsetConjuctive()` and
...
```{r Conjuctive, echo=TRUE}
skill <- c("High","Medium","Low")
eThetaFrame(list(S1=skill,S2=skill), c(S1=1.25,S2=.75), 0.33, "Conjuctive")
```

```{r Disjuctive, echo=TRUE}
skill <- c("High","Medium","Low")
eThetaFrame(list(S1=skill,S2=skill), c(S1=1.25,S2=.75), 0.33, "Disjuctive")
```
...
The _Offset Conjuctive_ and _Offset Disjunctive_ models use one
